...
function showProjAppFeatures(data) {
    if (data && data.features) {
        var arrlen = data.features.length;
        var rowbegin = "<tr>";
        var fldbegin = "<td>";
        var rowend = "</tr>";
        var fldend = "</td>";
        var result = "";
        for (var ix = 0; ix < arrlen; ix++) {
            var featrow = data.features[ix];
            var str1 = rowbegin.substr(0);
            var therow = str1.concat(fldbegin, featrow.name, fldend, fldbegin, featrow.newflg, fldend, fldbegin, featrow.estimate, fldend, fldbegin, featrow.risk, fldend, fldbegin, featrow.status, fldend, fldbegin, featrow.jiraepic, fldend, rowend);
            result += therow;
        }
    }
}
